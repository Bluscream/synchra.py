...
async def test_http_get_success(http_client, mock_aioresponse):
    mock_aioresponse.get("https://api.synchra.net/test", status=200, payload={"key": "value"})
...
async def test_http_error_handling(http_client, mock_aioresponse):
    mock_aioresponse.get("https://api.synchra.net/test", status=400, payload={"type": "bad_request", "message": "invalid parameters", "code": 400, "errors": []})
...
async def test_http_204_no_content(http_client, mock_aioresponse):
    mock_aioresponse.delete("https://api.synchra.net/test", status=204)
